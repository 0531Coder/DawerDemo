...
public class ProgressSubscriber<T> extends Subscriber<T> {
    ...
    public void onStart() {
        showProgressDialog();
        /*缓存并且有网*/
        if (api.isCache() && HttpUtil.isNetAvailable(RxRetrofitApp.getApplication())) {
             /*获取缓存数据*/
            ...
            if (CookieResult != null) {
                ...
                if (time < api.getCookieNetWorkTime()) {
                    if (mSubscriberOnNextListener.get() != null) {
                        mSubscriberOnNextListener.get().onNext(CookieResult.getResult(),api.getMothed());
                    }
                    onCompleted();
                    unsubscribe();
                }
        ...
        if (api.isCache()) {
            Observable.just(api.getUrl()).subscribe(new Subscriber<String>() {
                ...
                public void onNext(String s) {
                    ...
                    if (CookieResult == null) {
                        throw new HttpRunTimeException("网络错误");
                    }
                    long time = (System.currentTimeMillis() - CookieResult.getTime()) / 1000;
